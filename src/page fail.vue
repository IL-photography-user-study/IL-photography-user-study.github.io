          ...
          <a-col class="group-col" :xs="24" :sm="24" :md="12" :lg="12" :xl="12" :xxl="12" v-for="col in 2" :key="'group-' + ((row - 1) * 2 + col)">
            <template v-if="(currentGroup - 1) * 8 + (row - 1) * 2 + col <= totalGroups">
              ...
              >
                ...
                <!-- <template>
                  <div>
                    <a-select default-value="Image1" style="width: 120px" @change="handleChange">
                      <a-select-option value="Rank1">
                        1
                      </a-select-option>
                      <a-select-option value="Rank2">
                        2
                      </a-select-option>
                      <a-select-option value="Rank3">
                        3
                      </a-select-option>
                      <a-select-option value="Rank4">
                        4
                      </a-select-option>
                    </a-select>
                    <a-select default-value="Image2" style="width: 120px" @change="handleChange">
                      <a-select-option value="Rank1">
                        1
                      </a-select-option>
                      <a-select-option value="Rank2">
                    ...
                    <a-select default-value="Image3" style="width: 120px" @change="handleChange">
                      <a-select-option value="Rank1">
                        1
                      </a-select-option>
                      <a-select-option value="Rank2">
                    ...
                    <a-select default-value="Image4" style="width: 120px" @change="handleChange">
                      <a-select-option value="Rank1">
                        1
                      </a-select-option>
                      <a-select-option value="Rank2">
                ...
                <!-- <a-cascader
                    :options="getCascaderOptions((currentGroup - 1) * 8 + (row - 1) * 2 + col - 1, qIndex)"
                    multiple
                    :show-search="{ Cascaderfilter }"
                    placeholder="Select ranks (you can select multiple for equal rank)"
                    @change="value => onCascaderChange(value, (currentGroup - 1) * 8 + (row - 1) * 2 + col - 1, qIndex)"
                ...
                <!-- <div class="ranking-bar">
                  ...
                  <draggable
                    :list="groupRankings[(currentGroup - 1) * 8 + (row - 1) * 2 + col - 1][qIndex]"
                    @change="event => onRankingChange(event, (currentGroup - 1) * 8 + (row - 1) * 2 + col - 1, qIndex)"
            ...
            <div class="ranking-comment">
              ...
              <a-form-item>
                <a-textarea
                  :rows="2"
                  v-model="groupComments[(currentGroup - 1) * 8 + (row - 1) * 2 + col - 1][qIndex]"
...
export default {
  ...
  methods: {
    ...
    // onCascaderChange(value, groupIdx, qIndex) {
    //   // value : [[imgIndex, rank], ...]  
    //   // 保存到 groupRankings
    //   // this.$set(this.groupRankings[groupIdx], qIndex, value.map(v => ({ img: v[0], rank: v[1] })));
    //   this.$set(this.groupRankings[groupIdx], qIndex, value);
    // },

    // getCascaderOptions(groupIdx, qIndex) {
    //   if (!this.groupRankings[groupIdx] || !this.groupRankings[groupIdx][qIndex]) return [];

    //   const images = this.groupOrders[groupIdx];
    //   const selectedRanks = this.groupRankings[groupIdx][qIndex].map(item => item.img); 
    //   const remaining = images.filter(img => !selectedRanks.includes(img)); 
    ...
    //         label: `Image ${i}`,
    //         children: remaining
    //           .filter(j => j !== img && j !== i) // 下一级不能再选同一张
    //           .map(j => ({
    //             value: j,
    //             label: `Image ${j}`,
    //             children: remaining
    //               .filter(q => q !== img && q !== i && q !== j) // 下一级不能再选同一张
    //               .map(q => ({
    //                 value: q,
    //                 label: `Image ${q}`,
    ...
    submitForm() {
      ...
      const payload = {
        groups: this.groupRankings.map((groupQuestions, i) => ({
          groupIndex: i + 1,
          rankings: this.groupOriginalOrders[i].map(originalImg => {
            // 找到当前图片在下拉框中选择的 rank
            const rankedItem = groupQuestions.flat().find(item => item.img === originalImg);
            return {
              img: originalImg,
              rank: rankedItem ? rankedItem.rank : null
            };
          }),
